...
def update_user_profile():
    user = get_jwt_identity()
    try:
        required_keys = ["profile_image","zodiac_sign","location","gender","love_interest","date_born","description", "height"]
        if not all(key in request.json for key in required_keys):
            return jsonify({"message": "missing an 'profile_image', 'zodiac_sign', 'location', 'gender', 'love_interest', 'date_born','description', 'height' keys in json"}), 400
        else:
